...
class SHConvBlock(nn.Module):
    def __init__(self, Y, Y_inv, area, in_ch, out_ch, L, interval, nonlinear=None, fullband=True, bn=True):
        ...
        out_ch : int
            # of output channels.
        L : int
            Spectral bandwidth that supports individual component learning.
        interval : int
            Interval of anchor points.
...
class SPHARM_Net(nn.Module):
    def __init__(self, sphere, device, in_ch=3, n_class=32, C=128, L=80, D=3, interval=5, threads=1, verbose=False):
        ...
        C : int
            # of channels in the entry layer.
        L : int
            Spectral bandwidth that supports individual component learning.
        D : int
            Depth of encoding/decoding levels.
        interval : int
            Interval of anchor points.
        threads : int
        ...
        for i in range(D):
            ...
            self.down.append(SHConvBlock(Y, Y_inv, area, in_ch, out_ch, L, interval, nonlinear, i == 0)) # first layer - SHConv-FB Block, others - SHConv Block
            in_ch = out_ch
            out_ch *= ch_inc
        ...
        self.down.append(SHConvBlock(Y, Y_inv, area, in_ch, out_ch, L, interval, nonlinear, False)) # SHConv Block
        ...
        for i in range(D - 1):
            ...
            self.up.append(SHConvBlock(Y, Y_inv, area, in_ch, out_ch, L, interval, nonlinear, False)) # SHConv Block

        in_ch = out_ch * 2
        L *= 2
        if verbose:
            print("Up {}\t| C:{} -> {}\t| L:{}".format(D, in_ch, out_ch, L))
        self.up.append(SHConvBlock(Y, Y_inv, area, in_ch, out_ch, L, interval, nonlinear, True)) # SHConv-FB Block
        if verbose:
            print("Final\t| C:{} -> {}\t| L:{}".format(out_ch, n_class, L))
        self.final = SHConvBlock(Y, Y_inv, area, out_ch, n_class, L_in, interval, None, True) # SHConv-FB Block w/o ReLU (final layer)
    ...
    def forward(self, x):
        ...
        for i in range(len(self.up)):
            x = torch.cat([x, x_[-2 - i]], dim=1) # concatenate
            x = self.up[i](x)
